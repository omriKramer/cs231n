...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = x, prev_h, prev_c, Wx, Wh, i, f, o, g, tan_next_c
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, prev_h, prev_c, Wx, Wh, i, f, o, g, tan_next_c = cache
    N, H = dnext_h.shape

    do = dnext_h * tan_next_c
    dtan_next_c = dnext_h * o
    dnext_c = dtan_next_c * (1 - tan_next_c ** 2) + dnext_c
    df = dnext_c * prev_c
    dprev_c = dnext_c * f
    di = dnext_c * g
    dg = dnext_c * i
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dnext_h = dh[:, t, :] + dprev_h
        dx[:, t, :], dprev_h, dprev_c, dWx_tmp, dWh_tmp, db_tmp = lstm_step_backward(dnext_h, dprev_c, cache[t])
        dWx += dWx_tmp
        dWh += dWh_tmp
        db += db_tmp
