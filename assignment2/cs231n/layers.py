...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        xmu = x - sample_mean
        var = (xmu ** 2).sum(axis=0) / N
        ivar = 1 / np.sqrt(var + eps)
        xhat = xmu * ivar
        out = gamma * xhat + beta

        cache = gamma, xhat, xmu, ivar, var, eps
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, D = dout.shape
    gamma, xhat, xmu, ivar, var, eps = cache
    dbeta = dout.sum(axis=0)
    dgamma = np.sum(dout * xhat, axis=0)

    dxhat = dout * gamma
    divar = np.sum(xmu * dxhat, axis=0)
    dsqrtvar = -1 * divar / (var + eps)
...
def layernorm_backward(dout, cache):
    ...
    dxhat = np.transpose(dout * gamma)
    dx = np.transpose(ivar / D * (D * dxhat - np.sum(dxhat, axis=0) - xhat * np.sum(dxhat * xhat, axis=0)))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
        ...
        for f in range(F):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    out[n, f, h_out, w_out] = (np.sum(x_pad[n,
                                                      :,
            ...
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    ind = np.unravel_index(
                        np.argmax(x[n, c, h_start:h_start + pool_height, w_start:w_start + pool_width]),
                        (pool_height, pool_width)
                    )
                    dx[n, c, h_start + ind[0], w_start + ind[1]] = dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = gamma, xhat, xmu, ivar, var, eps, G
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    gamma, xhat, xmu, ivar, var, eps, G = cache
    ...
    dxhat = dxhat.reshape(N * G, D).T
    xhat = xhat.reshape(N * G, D).T
    dx = np.transpose(ivar / D * (D * dxhat - np.sum(dxhat, axis=0) - xhat * np.sum(dxhat * xhat, axis=0)))
