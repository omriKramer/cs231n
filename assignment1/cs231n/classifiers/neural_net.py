...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # shape (N, C).                                                             #
        #############################################################################
        l1 = X.dot(W1) + b1[np.newaxis, :]
        relu_out = np.maximum(0, l1)
        ...
        loss = -np.log(scores_norm[range(N), y]).sum()
        ...
        dW2 = relu_out.T.dot(dscores)
        dW2 += 2 * reg * W2

        drelu = dscores.dot(W2.T)

        dl1 = np.where(l1 > 0, drelu, 0)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            ind = np.random.choice(num_train, batch_size, replace=replace)
            X_batch = X[ind]
            y_batch = y[ind]
            #########################################################################
            #                             END OF YOUR CODE                          #
